...
data class ReviewCounts(
    ...
    val closed: Int get() = done + skipped + missed
...
fun reviewInsights(state: SlidingTasksState, today: LocalDate): ReviewInsights {
    ...
    val patterns = recentCards.groupBy { it.taskId }.map { (taskId, cards) ->
        TaskWindow(
            ...
            skipped = cards.count { it.status == CardStatus.DISMISSED },
    ...
    val mostSkipped = patterns.filter { it.skipped > 0 }
        .maxWithOrNull(compareBy<TaskWindow> { it.skipped }.thenBy { it.skipped.toDouble() / it.total })
        ?.let { ReviewPattern(it.title, it.skipped, it.total) }
    ...
    return ReviewInsights(counts(weekCards), mostMissed, mostSkipped, mostDone, trend, days)
}
